...
def execute_query(graph,g,id,i,ORDER_j,DATABASE,DATASET,QUERY,ITERATION,OBJECT_ARRAY,SID, PROP_NAME, SKIP_COMMIT){
	e = g.E(id).next();
	...
	query+=' RETURN n';
	t = System.nanoTime();
	c=graph.cypher(query).select('n').next();
...
if (SID == EDGE_LID_ARRAY.size()) { 
	order_j = 1;
	for (i in RAND_ARRAY) {
       execute_query(graph,g,EDGE_LID_ARRAY[i],i,order_j,DATABASE,DATASET,QUERY,ITERATION,EDGE_ARRAY,SID,PROPERTY_NAME, SKIP_COMMIT);
       order_j++;
	}
} else {
	execute_query(graph,g,EDGE_LID_ARRAY[SID],SID,0,DATABASE,DATASET,QUERY,ITERATION,EDGE_ARRAY,SID,PROPERTY_NAME, SKIP_COMMIT);
}
