...
def execute_query(graph,g,id,i,ORDER_j,DATABASE,DATASET,QUERY,ITERATION,OBJECT_ARRAY,SID, PROP_NAME, PROP_VAL,SKIP_COMMIT){
	...
	query+='" RETURN n';
	t = System.nanoTime();	
	e=graph.cypher(query).select('n').next();
...
if (SID == EDGE_LID_ARRAY.size()) {
    ...
    execute_query(graph,g,EDGE_LID_ARRAY[SID],SID,0,DATABASE,DATASET,QUERY,ITERATION,EDGE_ARRAY,SID, PROPERTY_NAME, PROPERTY_VALUE,SKIP_COMMIT);
}
