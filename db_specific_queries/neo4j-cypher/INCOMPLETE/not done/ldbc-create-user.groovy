...
//select max oid
t = System.nanoTime();
query="MATCH (n) WHERE n.xlabel='person' RETURN max(n.oid) as value";
oid=graph.cypher(query).select('value').next();
...
t = System.nanoTime();
query="MATCH (n) WHERE n.xlabel='person' RETURN max(n.iid) as value";
iid=graph.cypher(query).select('value').next();
...
//------- User Insertion -----
t = System.nanoTime();
query="CREATE (n {firstname: '"+firstNames[fNidx]+"', lastName: '"+lastNames[lNidx]+"', creationDate: '"+creationDates[cDidx]+"', birthday:'"+birthdays[bDidx]+"', locationIP:'"+locationIPs[iPidx+"', email:'"+firstNames[fNidx]+"."+lastNames[lNidx]+SID+"@mymail.com"+"', gender:'"+sex+"', browserUsed:'Opera', xlabel:'person', language:'si', oid:'"+newOid+"', iid:'person:"+newIid+"'}) RETURN id(n) as value";
new_v=graph.cypher(query).select('value').next();
...
t = System.nanoTime();
query="MATCH (n) WHERE n.type='city' AND n.xname>";
query+=placeNames[plNidx];
query+=" RETURN id(n) as value ORDER BY n.xname LIMIT 1;
pCity=graph.cypher(query).select('value').next();
...
query+=" AND id(b)=";
query+=pCity;
query+= CREATE (a)-[r:isLocatedIn]->(b) RETURN type(r)";
...
t = System.nanoTime();
query="MATCH (n) WHERE n.type='company' AND n.xname>";
query+=companyNames[cpNidx];
query+=" RETURN id(n) as value ORDER BY n.xname LIMIT 1;
pWork=graph.cypher(query).select('value').next();
//pWork = g.V().has('type', 'company').filter(has('xname',gt(companyNames[cpNidx]))).barrier().order().by('xname', incr).limit(1).next()
query="MATCH (a), (b) WHERE id(a)=";
query+=new_v;
query+=" AND id(b)=";
query+=pWork;
query+= CREATE (a)-[r:workAt]->(b) RETURN type(r)";
graph.cypher(query).next();
...
t = System.nanoTime();
query="MATCH (n) WHERE n.type='university' AND n.xname>";
query+=placeNames[plNidx];
query+=" RETURN id(n) as value ORDER BY n.xname LIMIT 1;
pEdu=graph.cypher(query).select('value').next();
//pEdu = g.V().has('type', 'university').filter(has('xname',gt(placeNames[plNidx]))).barrier().order().by('xname', incr).limit(1).next()
query="MATCH (a), (b) WHERE id(a)=";
query+=new_v;
query+=" AND id(b)=";
query+=pEdu;
query+= CREATE (a)-[r:studyAt]->(b) RETURN type(r)";
graph.cypher(query).next();
...
t = System.nanoTime();
cc=0
query="MATCH (n) WHERE n.firstName>=";
query+=peopleNames[peNidx];
query+=" RETURN id(n) as value ORDER BY n.firstName LIMIT 1";
friend1 = graph.cypher(query).select('value');
...
if(friend1.hasNext()){
  ...
  query+=" AND id(b)=";
  query+=friend1;
  query+=" CREATE (a)-[r:knows]->(b) RETURN TYPE(r) as value";
  graph.cypher(query).select('value').next();
...
result_row = [ DATABASE, DATASET, QUERY+"-friend", String.valueOf(SID), ITERATION, String.valueOf(ORDER_j), String.valueOf(exec_time), "friend1 " + cc ];
println result_row.join(',');
...
t = System.nanoTime();
cc=0
query="MATCH (n) WHERE n.lastName>=";
query+=peopleNames[peNidx];
query+=" RETURN id(n) as value ORDER BY n.firstName LIMIT 1";
friend2 = graph.cypher(query).select('value');
...
result_row = [ DATABASE, DATASET, QUERY+"-friend", String.valueOf(SID), ITERATION, String.valueOf(ORDER_j), String.valueOf(exec_time), "friend2 " +  cc];
println result_row.join(',');
...
cc=0
query="MATCH (n) WHERE n.lastName=";
query+=lastNames[lNidx];
query+="AND n.firstName!=";
query+=firstNames[fNidx];
query+=" RETURN id(n) as value ORDER BY n.firstName DESC LIMIT 1";
friend3 = graph.cypher(query).select('value');
...
result_row = [ DATABASE, DATASET, QUERY+"-friend-and", String.valueOf(SID), ITERATION, String.valueOf(ORDER_j), String.valueOf(exec_time), "friend3 " + cc];
println result_row.join(',');
...
t = System.nanoTime();
tags=[]
query="MATCH (n)-[r:knows]->(f)-[s:hasInterest]->(i) WHERE n.oid=";
query+=newOid;
query+=" RETURN i ORDER BY count(s) DESC LIMIT 10";
tt=graph.cypher(query).select('i').fill(tags);
...
query+=" AND b.oid IN ";
query+=tags;
query+=" CREATE (a)-[r:hasInterest]->(b) RETURN type(r) as value";
graph.cypher(query).select('value').next();
...
result_row = [ DATABASE, DATASET, QUERY+"-q1", String.valueOf(SID), ITERATION, String.valueOf(ORDER_j), String.valueOf(exec_time),  llist.size()];
println result_row.join(',');
