...
Thread.sleep(1000*60*60); //it was 1000*60*60
...
try{
  if(DEBUG){
    ...
    for(int i=0; i < NODE_ARRAY.size(); i++){
      ...
      query="MATCH (n:vertex) WHERE n.";
      query+=INDEXABLE_ATTRIBUTE;
      query+="= ";
      query+=NODE_ID;
      query+=" RETURN count(n) as cnt";
      count1+= graph.cypher(query).select('cnt').next();
    ...
    count2 = 0
    nEx = f.max_uid
    println "first nEx is "+nEx;
    for(int i=0; i < NODE_ARRAY.size(); i++){
      nEx = f.infer_type(nEx+1);
      //println "new nEx is "+nEx;
      query="MATCH (n:vertex) WHERE n.";
      query+=INDEXABLE_ATTRIBUTE;
      query+="= ";
      query+=nEx;
      query+=" RETURN count(n) as cnt";
      count2 += graph.cypher(query).select('cnt').next();
