...
def execute_query(g,id,i,ORDER_j,DATABASE,DATASET,QUERY,ITERATION,OBJECT_ARRAY,LABEL_ARRAY,SID,DEPTH){
	v = g.V(id);


	t = System.nanoTime();
    //count = v.repeat(both(*LABEL_ARRAY).simplePath()).times(DEPTH).emit().count().next();
    count = v.repeat(both(*LABEL_ARRAY).where(without("x")).aggregate("x")).times(DEPTH).cap("x").next().size();
...
if (SID == NODE_LID_ARRAY.size()) { 
	order_j = 1;
	for (i in RAND_ARRAY) {
        execute_query(g,NODE_LID_ARRAY[i],i,order_j,DATABASE,DATASET,QUERY,ITERATION,NODE_ARRAY,LABEL_ARRAY,SID,BFS_N);
        order_j++;
	}
} else {
    execute_query(g,NODE_LID_ARRAY[SID],SID,0,DATABASE,DATASET,QUERY,ITERATION,NODE_ARRAY,LABEL_ARRAY,SID,BFS_N);
}
