...
/* Extract nodes and edges collections from a GraphSON file.
   Required by arangodb, that is unable to parse "big" GraphSON. */
...
func processRow(line string, isEdge bool) map[string]string {
	...
	/*
		for _, element := range strings.Split(strings.Trim(line, "{}"), ",") {
			tuple := strings.SplitN(element, ":", 2)
			if len(tuple) == 2 {
				dict[strings.Trim(tuple[0], "\"")] = tuple[1]
			} else {
				fmt.Printf("Strange line: %s\n", element)
			}
		}
	*/
