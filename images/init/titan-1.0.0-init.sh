...
if [[ -z ${TITAN_JAVA_OPTS+x} ]]; then
    echo "WARN:  TITAN_JAVA_OPTS is  NOT set. We'll get some defaults!" >> ${RUNTIME_DIR}/errors.log
    export TITAN_JAVA_OPTS='-Xms1G -Xmn128M -Xmx8G'
fi
...
if [[ "$QUERY" == *loader.groovy ]]; then
    ...
    # THIS IS NOT EFFECTIVE BUT WE KEEP IT
    rm -v $TITAN_HOME/conf/log4j-console.properties     >> ${RUNTIME_DIR}/errors.log
