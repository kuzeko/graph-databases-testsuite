...
if ! [[ -z ${JAVA_OPTIONS+x} ]]; then
    echo "WARN:  JAVA_OPTIONS is set but this breaks Janus's version of gremlin console! Set JANUS_JAVA_OPTS instead!" >> ${RUNTIME_DIR}/errors
    JANUS_JAVA_OPTS=$JAVA_OPTIONS
    unset JAVA_OPTIONS
fi

if [[ -z ${JANUS_JAVA_OPTS+x} ]]; then
    echo "WARN:  JANUS_JAVA_OPTS is  NOT set. We'll get some defaults!" >> ${RUNTIME_DIR}/errors
...
if [[ "$QUERY" == *loader.groovy ]]; then
    ...
    # THIS IS NOT EFFECTIVE BUT WE KEEP IT
    rm -v $JANUS_HOME/conf/log4j-console.properties     >> ${RUNTIME_DIR}/errors
    ...
    if [[  -f ${RUNTIME_DIR}/presampled/"${DNAME}_props.txt"  ]] && [[  -f ${RUNTIME_DIR}/presampled/"${DNAME}_labels.txt"  ]] ; then
         ...
         echo "Extracting properties and labels ${DATASET/%2/}" >> ${RUNTIME_DIR}/errors
         echo "Extracting properties and labels ${DATASET/%2/}" >> ${RUNTIME_DIR}/errors
         go run /extract_schema.go -i "${DATASET/%2/}" -l /labels.txt -p /props.txt >> ${RUNTIME_DIR}/errors
         echo "Finished Extraction of properties and labels" >> ${RUNTIME_DIR}/errors
    fi

    echo "JanusGraph schema creation" >> ${RUNTIME_DIR}/errors
    gremlin.sh -e /janusgraph-create-schema.groovy 2>> "$RUNTIME_DIR/errors" 1>> "$RUNTIME_DIR/results"
...
if [[ "$QUERY" == *loader.groovy ]]; then
    if [[ -z ${USE_INDEX+x} ]]; then
        ...
        gremlin.sh -e /janusgraph-drop-index.groovy 2>> "$RUNTIME_DIR/errors" 1>> "$RUNTIME_DIR/results"
