 ...
 * Q 15 -
 */
public class EdgePropertySearch extends Simple {
	...
	protected Collection<TimingCheckpoint> query(GraphTraversalSource gts, Simple.QParam params, Sample sample, Dataset ds, long thread_id, GenericIndexMgm imgm, GenericShell shell) {
		...
		// Long count = gts.E().has(params.propName, StrSerialize.deserialize(params.propClass, params.propValue)).count().next();
		Long count = gts.E().has(QSupportSchema.ECommonProperty, QSupportSchema.ECommonValue).count().next();
		long end = System.currentTimeMillis();

		return List.of(new TimingCheckpoint(end - start, String.format("Found %s edges with: %s=%s (should be #sampled edges)",
				count, QSupportSchema.ECommonProperty, QSupportSchema.ECommonValue), params));
	}
}
