 ...
 * Q 26 -
 */
public class NNincoming extends NN {
	...
	protected Collection<TimingCheckpoint> query(GraphTraversalSource gts, NN.QParam params, Sample sample, Dataset ds, long thread_id, GenericIndexMgm imgm, GenericShell shell) {
		...
		long count = gts.V(sample.mapping.nodes.get(params.node)).inE().count().next();
		long end = System.currentTimeMillis();

		return List.of(new TimingCheckpoint(end-start, String.format("%s has %d incoming edges",
