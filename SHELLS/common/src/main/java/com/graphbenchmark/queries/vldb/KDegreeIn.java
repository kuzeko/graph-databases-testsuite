 ...
 * Q 32 -
 */
public class KDegreeIn extends Simple {
	...
	protected Collection<TimingCheckpoint> query(GraphTraversalSource gts, Simple.QParam params, Sample sample, Dataset ds, long thread_id, GenericIndexMgm imgm, GenericShell shell) {
		...
		long count = gts.V().where(__.inE().count().is(P.gte(K))).count().next();
		end = System.currentTimeMillis();
		return List.of(new TimingCheckpoint(end - start, String.format("nodes with in-k > %d: %d", K, count), params));
	}
